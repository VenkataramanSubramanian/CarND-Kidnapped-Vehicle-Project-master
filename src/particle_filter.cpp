...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
	...
	// Initialization for each particle with the GPS values
...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
	...
	if ( fabs(yaw_rate) < 0.00001 ) //The Vehicle movement with no change in yaw_rate value between time stamps
	{ 
		particles[i].x += velocity * delta_t * cos( theta );
		particles[i].y += velocity * delta_t * sin( theta );

		//We use linear equation to update the position with no change in yaw_rate value
	} 
	else 
	{
		//Since the angle changes we use the below equations
...
void ParticleFilter::dataAssociation(std::vector<LandmarkObs> predicted, std::vector<LandmarkObs>& observations) {
	...
	//We find the landmark coordinates that are close those the particle map observed coordinates using neareast neighbours
	...
	{ 
		...
		//Applyng the KNN technique
		for (unsigned j = 0; j < predicted.size(); j++ ) 
		{ 
			double xDistance = abs(observations[i].x - predicted[j].x);
			double yDistance = abs(observations[i].y - predicted[j].y);

			double distance = xDistance + yDistance; //No need to square as the value doesnt matter
...
std::vector<LandmarkObs> ParticleFilter::mapcoordinates(double x, double y, double theta,const std::vector<LandmarkObs> observations)
{
	
	// Transform the observed value into map coordinates. so the partcle can find which landmark it corresponds to
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
	...
	for (int i = 0; i < num_particles; i++) 
	{
		...
		//Picking the landmarks the particle could have observed
		for(unsigned int j = 0; j < map_landmarks.landmark_list.size(); j++) 
		{
			...
			if ( dX*dX + dY*dY <= sensor_range_2 ) //If the Vector distance is not in sensors range it is not considered
			{
				inRangeLandmarks.push_back(LandmarkObs{ id, landmarkX, landmarkY });
			}
		}

		//Transforming vehicle coordinate system into map coordinate system
		vector<LandmarkObs> mappedObservations;
		mappedObservations=mapcoordinates(particles[i].x,particles[i].y,particles[i].theta,observations);
		...
		{
			//Taking the first observation value
			...
			//Finding the landmark values that map the id which we gavce to mappedobservation in the function	
			while(k < inRangeLandmarks.size()) 
			{
